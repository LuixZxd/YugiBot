...
if (new Date - user.lastmiming < 10000) return await conn.reply(m.chat, `*ESPERA UNOS MINUTOS PARA USAR OTRO COMANDO*`,  m)
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `PARA USAR ESTE COMANDO RESPONDE A UN VIDEO O IMAGEN CON EL COMANDO  .s`
      let out
      try {
    ...
    else throw `PARA USAR ESTE COMANDO RESPONDE A UN VIDEO O IMAGEN CON EL COMANDO  .s`
  }
